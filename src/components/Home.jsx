...
const Home = ({selectedCategory}) => {
  ...
  const getNews = async () => {
    try {
      const res = await fetch(`https://newsapi.org/v2/top-headlines?category=${selectedCategory}&apiKey=${API_KEY}`);
            ...
            <div key={index} className='shadow mx-5 mb-5 flex rounded-2xl bg-[rgb(41, 42, 45)] p-2 '>
              ...
              <div className='flex flex-col max-w-100'>
                <a href={article.url} target='_blank'>
                  <h4 className='font-bold text-sm md:text-base text-black'>{article.title}</h4>
                  <p className="line-clamp-2 text-xs md:text-base md:line-clamp-4 text-gray-700">{article.description}</p>
